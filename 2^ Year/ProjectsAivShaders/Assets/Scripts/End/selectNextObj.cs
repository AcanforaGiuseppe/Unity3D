...
public class selectNextObj : MonoBehaviour {
	...
	void Start () {
        ...
        objSelShader = Shader.Find("Custom/_textureOutline");
        objNotSelShader = Shader.Find("Custom/_texture");
    }
    ...
    void Update () {
        if (Input.GetKeyDown(nextKey))
        {
            ...
            currObjSelSMat = currObjSelected.GetComponent<Renderer>().sharedMaterial;
            currObjSelSMat.shader = objSelShader;
        }
        if (currObjSelected != null)
            currObjSelSMat.SetFloat("_OutlineBorder", Mathf.Abs(Mathf.Sin(Time.time*2)) / 10.0f);
    }
